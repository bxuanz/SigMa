...
# =========================================================================
# [SigMa] Strategy 1: First-Principles Adaptive Scheduler
# =========================================================================
# def get_adaptive_scale(t: float, scale_factor: float) -> float:
#     """
#     完全基于分辨率尺度定律 (Scaling Law) 自动推导，无人工超参数。
#     """
#     # 1. 几何中心律 (Geometric Center Law)
#     # 使用平方根倒数，为高分辨率(如4x)预留更多细节生成时间
#     t_center = 1.0 / math.sqrt(scale_factor)
    
#     # 2. 线性硬度律 (Linear Hardness Law)
#     # 移除 2.5 系数。假设相变陡峭程度应与分辨率线性相关。
#     # Hardness = S。这是最自然的物理基准。
#     dynamic_hardness = scale_factor
    
#     # 3. Sigmoid 相变模拟
#     x = (t - t_center) * dynamic_hardness
...
class FluxAttnProcessor:
    ...
    def __call__(
        ...
        # [修改] 移除了 Image Stream Query 的缩放逻辑
        ...
        if attn.added_kv_proj_dim is not None:
            ...
            # [修改] 移除了 Text Stream Query 的缩放逻辑
...
def get_1d_rotary_pos_embed(
    ...
    if yarn and max_pe_len is not None and max_pe_len > ori_max_pe_len:
        ...
        # -----------------------------------------------------------
        if sigma:
            # 1. 调度器: 使用 scale_val (64.0) 来计算中心点
            # 注意：如果你的 get_adaptive_scale 内部是基于 scale 的，
            # 这里的 scale_val 变大了，t_center 会变小 (相变会更早)，这也是合理的。
            adaptive_alpha = get_adaptive_scale(current_timestep, scale_val)
            
            # [Track 2: 强度计算] 
            # 关键点来了！我们不能用 log2(64)=6.0，太强了。
            # 我们需要 log2(4)=2.0。
            # 所以这里我们手动换算一下“物理倍率”：
            REAL_RES_BASE = 64
            physical_scale = max_pe_len.item() / REAL_RES_BASE
            physical_scale = max(1.0, physical_scale)
            
            # 这样算出来的 intensity 就是 2.0 (对应 4K)
            beta_intensity = max(1.0, math.log2(physical_scale))
            
            # 计算最终 Boost
            beta_boost = beta_intensity * adaptive_alpha
            
            # 应用 Boost
            beta_0 = beta_0 ** beta_boost
            beta_1 = beta_1 ** beta_boost
        ...
        if sigma:
            # Gamma: 保持 1.0 (不增强，解决多头)
            gamma_intensity = 0.9
            gamma_boost = gamma_factor * adaptive_alpha

            gamma_0 = gamma_0 ** gamma_boost
            gamma_1 = gamma_1 ** gamma_boost
...
class FluxTransformer2DModel(
    ...
    """
    The Transformer model introduced in Flux with SigMa support.
    """
    ...
    def forward(
        ...
        # [安全修复] 处理 timestep，兼容 Batch Size > 1 的情况
        if timestep.numel() > 1:
            current_t = timestep.float().mean().item()
        else:
            current_t = timestep.item()
        
        # 将归一化的时间步传递给位置编码模块 (用于 SigMa)
        self.pos_embed.set_timestep(current_t)
        ...
        # 恢复 timestep 为 Time Embedding 需要的数值范围 (通常是 0~1000)
